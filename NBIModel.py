...
class NBIModel(ForceModel):
  ...
  def initDistribution(self):
    ...
    #  if np.isnan(NBIModel.phi[j]):
    #    NBIModel.phi[j] = 0

    print('NOTE: Above warning has been resolved.')
    
    #for j in range(nmax):
    #        print(NBIModel.mass_halo[j])
    #        print(NBIModel.rad[j])
  ...
  def diffeq(self,x,f):
    ...
    for i in range(n):
      ...
      #Precaution against squaring negative numbers
      if tmp1 < 0:
        tmp1 = -tmp1
      ...
      r22d = np.square(tmp1) + np.square(tmp2) + np.square(tmp3)
      ...
      #Precation against squaring negative numbers
      if tmp1 < 0:
        tmp1 = -tmp1
      ...
      r21d = np.square(tmp1) + np.square(tmp2) + np.square(tmp3);
